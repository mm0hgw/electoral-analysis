...
ultraCounter <- function(...){
	...
	if(length(x)>1)return(lapply(x,out$index,LAPPLYFUN=LAPPLYFUN))
		stopifnot(x%%1==0)
	...
	out$revIndex <- function(...){
		...
		stopifnot(all(sapply(seq_along(args),function(x){args[[x]] %in% out$elems[[x]]})))
		sum(sapply(seq_along(args),function(x){which.max(args[[x]]==out$elems[[x]])-1})*out$colvals[-1])+1
	}
	out$sample <- function(k=1e3){
...
lapply(seq(5)*1e1,uc$index,sapply)
lapply(seq(5)*1e2,uc$index,sapply)
